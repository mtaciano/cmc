...
TreeNode *repeat_stmt(void) {
    TreeNode *t = new_StmtNode(RepeatK);
    match(REPEAT);
    if (t != NULL)
...
TreeNode *assign_stmt(void) {
    TreeNode *t = new_StmtNode(AssignK);
    if ((t != NULL) && (token == ID))
        t->attr.name = copy_string(tokenString);
    match(ID);
    match(ASSIGN);
    if (t != NULL)
        t->child[0] = exp();
...
TreeNode *write_stmt(void) {
    TreeNode *t = new_StmtNode(WriteK);
    match(WRITE);
    if (t != NULL)
        t->child[0] = exp();
    return t;
